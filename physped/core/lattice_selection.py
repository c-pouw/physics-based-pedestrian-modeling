"""This module contains functions to select a single point or a range of
values in the configuration file.

The only purpose is to let the user make a selection along the discretization
lattice.
"""
...
# ! The functions below are used to select a single point in the config file


def validate_value_within_lattice(
    selected_value: float, bins: dict, dimension: str
) -> None:
    ...
    outside_lattice = (selected_value < left_bound) or (
        selected_value > right_bound
    )
    if outside_lattice:
        raise ValueError(
            f"Selected {dimension} value ({selected_value}) is outside the "
            f"lattice. Please select a value within the range "
            f"[{left_bound},{right_bound}]."
        )


def validate_point_within_lattice(
    selected_point: DictConfig, grid_bins: dict
) -> None:
...
def get_boundaries_that_enclose_the_selected_bin(
    bin_index: int, bins: np.ndarray
) -> dict:
...
def evaluate_selection_point(config: DictConfig) -> DictConfig:
    ...
    selected_point.theta_periodic = periodic_angular_conditions(
        selected_point.theta, grid_bins["theta"]
    )
    validate_point_within_lattice(selected_point, grid_bins)
    selected_point.x_index = digitize_coordinates_to_lattice(
        selected_point.x, grid_bins["x"]
    )
    selected_point.y_index = digitize_coordinates_to_lattice(
        selected_point.y, grid_bins["y"]
    )
    selected_point.r_index = digitize_coordinates_to_lattice(
        selected_point.r, grid_bins["r"]
    )
    selected_point.theta_index = digitize_coordinates_to_lattice(
        selected_point.theta_periodic, grid_bins["theta"]
    )
    selected_point.k_index = digitize_coordinates_to_lattice(
        selected_point.k, grid_bins["k"]
    )
    return config


# ! The functions below are used to select a range of values in the config file


def is_selected_range_within_grid(
    selected_range: OmegaConf, grid_bins: dict
) -> None:
...
def is_range_decreasing(selected_range: OmegaConf):
    ...
    if range_decreasing:
        raise ValueError(
            "The selected range is invalid. The range must be increasing."
        )
...
def get_boundaries_that_enclose_the_selected_range(
    selected_range: OmegaConf, bins: dict
) -> dict:
...
def evaluate_selection_range(config: DictConfig) -> DictConfig:
    ...
    grid_bins = config.params.grid.bins
    selected_range.theta_periodic = periodic_angular_conditions(
        selected_range.theta, grid_bins["theta"]
    ).tolist()

    is_selected_range_valid(selected_range)
    is_selected_range_within_grid(selected_range, grid_bins)
    selected_range.x_indices = digitize_coordinates_to_lattice(
        selected_range.x, grid_bins["x"]
    ).tolist()
    selected_range.y_indices = digitize_coordinates_to_lattice(
        selected_range.y, grid_bins["y"]
    ).tolist()
    selected_range.r_indices = digitize_coordinates_to_lattice(
        selected_range.r, grid_bins["r"]
    ).tolist()
    selected_range.theta_indices = digitize_coordinates_to_lattice(
        selected_range.theta_periodic, grid_bins["theta"]
    ).tolist()
    selected_range.k_indices = digitize_coordinates_to_lattice(
        selected_range.k, grid_bins["k"]
    ).tolist()
