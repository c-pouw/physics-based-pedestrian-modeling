...
def sample_trajectory_origins_from_trajectory_state_n(
    parameters: dict,
    measured_trajectories: pd.DataFrame,
    state_n,
    ...
    if state_n == -1:
        ...
        states_to_sample_from = (
            measured_trajectories.groupby("Pid")
            .apply(lambda x: x.sample(1))
            .reset_index(drop=True)
        )
    elif state_n >= 0:
        # Sample from state n along each path
        states_to_sample_from = measured_trajectories[
            measured_trajectories["k"] == state_n
        ].copy()

    # Make sure that the potential is defined for the states we sample
    states_to_sample_from = potential_defined_at_slow_state(
        states_to_sample_from, piecewise_potential
    )
    states_to_sample_from = states_to_sample_from[
        states_to_sample_from["potential_defined"]
    ].copy()
...
def potential_defined_at_slow_state(
    ...
    # TODO : Move this function to another module. Change the
    # slow_grid_indices to lists rather than tuples
...
def sample_trajectory_origins_from_heatmap(
    piecewise_potential: PiecewisePotential, parameters: dict
) -> np.ndarray:
    sampled_origins = sample_from_ndarray(
        piecewise_potential.histogram[..., 0], parameters.simulation.ntrajs
    )
    sampled_origins = np.hstack(
        (sampled_origins, np.zeros((sampled_origins.shape[0], 1), dtype=int))
    )
    sampled_origins = convert_grid_indices_to_coordinates(
        piecewise_potential, sampled_origins
    )
...
def sample_from_ndarray(
    origin_histogram: np.ndarray, N_samples: int = 1
) -> np.ndarray:
...
def random_uniform_value_in_bin(
    lattice_indices: List[int], bins: np.ndarray
) -> np.ndarray:
    """Generate random uniform values within each bin.

    ! Used for simulation purposes

    This function is used to sample origins from a histogram. Given the index
    of the sampled lattice cell the function will return a real-world variable.

    For example:
    with bins v_r = [0,0.5,1] and lattice_indices [1] the function returns a
    random uniformly sampled value between 0.5 and 1.
...
def convert_grid_indices_to_coordinates(
    potential_grid: PiecewisePotential, X_0: np.ndarray
) -> np.ndarray:
    ...
    # xf, yf, rf, thetaf,
    # k = (grids.bins[dim][X_0[:,i]] for i, dim in enumerate(grids.dimensions))
    ...
    thetaf = random_uniform_value_in_bin(
        X_0[:, 3], potential_grid.bins["theta"]
    )
