...
angular_bins_1 = np.array(
    [np.pi / 2, np.pi, 3 * np.pi / 2, 2 * np.pi, 5 * np.pi / 2]
)
...
test_angles = [
    (angular_bins_1, angles_1, expected_angles_1),
    (angular_bins_2, angles_2, expected_angles_2),
]
...
def test_periodic_angular_conditions(angular_bins, angles, expected_angles):
    ...
    Args:
        angular_bins: An array with bin edges ranging over a
        total of 2 * np.pi.
