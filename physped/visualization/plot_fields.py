...
def plot_colorbar(
    ax: plt.Axes, cs: mpl.contour.QuadContourSet, label: str
) -> plt.Axes:
    """Plot the colorbar."""
    cbar = plt.colorbar(
        cs,
    ...
    # label = "$\\Delta V = 0.023\\log{\\left[\\mathbb{P}"
...
def calculate_offsets(hist, axis: Tuple = (2, 3, 4)):
    ...
    offset = offset_parameter * (
        -np.log(pos_hist) + np.log(np.nansum(pos_hist))
    )
    return offset
...
def create_force_fields(grids: dict, sliced_histogram: np.ndarray) -> dict:
    ...
    X, Y = np.meshgrid(
        grids.bin_centers["x"], grids.bin_centers["y"], indexing="ij"
    )
    fields["X"] = X
    fields["Y"] = Y
...
def plot_force_field_of_selection(grids, params, selection):
    ...
    slices = [
        tuple(grid_selection[dim]["grid_ids"]) for dim in grids.dimensions
    ]
