...
xbins1_midpoints = (xbins1[1:] + xbins1[:-1]) / 2
df["xcuts1"] = pd.cut(df.x, bins=xbins1, labels=xbins1[:-1])


def fit_func(x, pot0, beta, xmu):
    return beta * (x - xmu) ** 2 + pot0


for xi in xbins1:
    subset = df[df["xcuts1"] == xi]
    if len(subset) < 2:
        continue
    # plt.plot(subset.x, subset.y, "o")

    popt, _ = curve_fit(fit_func, subset.x, subset.y, p0=[0, 1, 0])
...
def fit_function_group(xi_group):
    return lambda x, a, b, c: a * (x - xi_group) ** 2
...
fig, axs = plt.subplots(1, 1, figsize=(3.54, 3), sharey=False, sharex=True)
ax = axs[0, 0]
